...
RSpec.describe Zendesk do 
	...
	describe 'issue processing' do
		...
		it 'parses issue array to remove irrelevant issues based on label' do
			issues = get_issues( 'TaylorBoudreau/TaylorBoudreau.github.io', 'zendesk' )
			puts issues.inspect
		...
		it 'does not include issues where the label does not match' do
			issues = get_issues( 'TaylorBoudreau/TaylorBoudreau.github.io', 'this_is_not_a_label_that_will_ever_exist' ) 
		...
		it 'that creates an array of clean issues' do
			clean_issues = clean_array_for_zendesk( @dirty_issue_array )
